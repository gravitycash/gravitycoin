...
def main():
    ...
    if not (enable_wallet and enable_utils and enable_gravitycoind):
        print("No functional tests to run. Wallet, utils, and gravitycoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if gravitycoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "gravitycoind"]) is not None:
            print("%sWARNING!%s There is already a gravitycoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/gravitycoind' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie gravitycoinds, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `gravitycoin-cli help` (`rpc_interface.txt`).

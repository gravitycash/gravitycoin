...
over /Applications/Bitcoin-Qt (on Mac) or gravitycoind/gravitycoin-qt (on Linux).
...
gravitycoind makes it easier for experienced open source developers to contribute 
to the project.
...
Another change in the 0.9 release is moving away from the gravitycoind executable
functioning both as a server and as a RPC client. The RPC client functionality
("tell the running bitcoin daemon to do THIS") was split into a separate
executable, 'gravitycoin-cli'. The RPC client code will eventually be removed from
gravitycoind, but will be kept for backwards compatibility for a release or two.
...
- Add '-disablewallet' mode to let gravitycoind run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with gravitycoind
...
- OS X: Fix gravitycoin-qt startup crash when clicking dock icon
...
- Add separate gravitycoin-cli client

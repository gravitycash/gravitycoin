...
#man pages
Source20:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/gravitycoind.1
Source21:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/gravitycoin-cli.1
Source22:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/gravitycoin-qt.1
...
The gravitycoin-cli utility allows you to communicate and control a bitcoin daemon
...
mv %{buildroot}%{_bindir}/gravitycoind %{buildroot}%{_sbindir}/gravitycoind

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoin.conf
d /run/gravitycoind 0750 bitcoin bitcoin -
EOF
...
PID_FILE="/run/gravitycoind/gravitycoind.pid"
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/gravitycoind -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/gravitycoind.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/gravitycoin-cli.1
%if %{_buildqt}
install -p %{SOURCE22} %{buildroot}%{_mandir}/man1/gravitycoin-qt.1
%endif
...
%attr(0755,root,root) %{_bindir}/gravitycoin-qt
...
%attr(0644,root,root) %{_mandir}/man1/gravitycoin-qt.1*
%endif
...
%attr(0755,root,root) %{_sbindir}/gravitycoind
...
%attr(0644,root,root) %{_mandir}/man1/gravitycoind.1*

%files utils
%defattr(-,root,root,-)
%license COPYING
%doc COPYING bitcoin.conf.example doc/README.md
%attr(0755,root,root) %{_bindir}/gravitycoin-cli
%attr(0755,root,root) %{_bindir}/bitcoin-tx
%attr(0755,root,root) %{_bindir}/bench_bitcoin
%attr(0644,root,root) %{_mandir}/man1/gravitycoin-cli.1*
